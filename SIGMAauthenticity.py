...
true_venue_labels = ['news', 'interview', 'television', 'show', 'speech', 'reporters', 'debate',
                     'newsletter', 'press', 'CNN', 'ABC', 'CBS', 'video', 'conference', 'official', 'book']
...
class VerifiableAuthenticity:
    ...
    def __init__(self):
        ...
        val1 = (dataTrain['true_score']+dataTrain['mostly_true'] +
                dataTrain['half_true']).astype(int)
        val2 = (dataTrain['mostly_false']+dataTrain['false_score'] +
                dataTrain['pants_on_fire']).astype(int)
        ...
        dataTrain[labelcolname] = dataTrain.apply(
            lambda row: self.simplify_statement_label(row['Label']), axis=1)

        # Calculating venue_score and adding venue score to the dataset
        labelcolname = 'venue_score'
        dataTrain[labelcolname] = dataTrain.apply(
            lambda row: self.simplify_venue_label(row['venue']), axis=1)
        ...
        tfidf_vectorizer = TfidfVectorizer(
            stop_words='english', ngram_range=(1, 2), use_idf=True, smooth_idf=True)
        tfidf_vectorizer_vectors = tfidf_vectorizer.fit_transform(
            dataTrain['Statement'].values)
        # tfidf.vocabulary_
        ...
        statement_accuracy = metrics.accuracy_score(
            dataTest['Label'], predictions_logReg)
    ...
    def cleanDataset(self, dataTrain):
        dataTrain['true_score'] = dataTrain['true_score'].str.split(
            " ", 1, expand=True)
        dataTrain['mostly_true'] = dataTrain['mostly_true'].str.split(
            " ", 1, expand=True)
        dataTrain['half_true'] = dataTrain['half_true'].str.split(
            " ", 1, expand=True)
        dataTrain['mostly_false'] = dataTrain['mostly_false'].str.split(
            " ", 1, expand=True)
        dataTrain['false_score'] = dataTrain['false_score'].str.split(
            " ", 1, expand=True)
        dataTrain['pants_on_fire'] = dataTrain['pants_on_fire'].str.split(
            " ", 1, expand=True)
    ...
    def predict(self, statement='', venue=''):
        ...
        binaryValue, probValue = self.getAuthenticityScoreByStatement(
            concatStatement)
